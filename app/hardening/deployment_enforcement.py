...
SUPPORTED_RUNTIME_AUTHORITY_VERSIONS = ("RA-1.0", "ASVH-RA-1.0")
...
def _valid_bind_semantics(bind: ProtectedClinicalBind, commit: ExactClinicalCommit) -> bool:
    ...
    if bind.schema_version != SUPPORTED_PROTECTED_BIND_SCHEMA:
        return False
    if bind.runtime_authority_version not in SUPPORTED_RUNTIME_AUTHORITY_VERSIONS:
        return False
...
class DeploymentEnforcer:
    ...
    protected-bind schema and one of the explicitly declared reference-harness
    runtime-authority versions, and retain the exact commit materiality and
